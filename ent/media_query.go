...
func (mq *MediaQuery) QueryEpisodes() *EpisodeQuery {
	query := (&EpisodeClient{config: mq.config}).Query()
...
func (mq *MediaQuery) Clone() *MediaQuery {
	...
	return &MediaQuery{
		...
		withEpisodes: mq.withEpisodes.Clone(),
...
func (mq *MediaQuery) WithEpisodes(opts ...func(*EpisodeQuery)) *MediaQuery {
	query := (&EpisodeClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withEpisodes = query
	return mq
}
...
func (mq *MediaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Media, error) {
	...
	if query := mq.withEpisodes; query != nil {
		if err := mq.loadEpisodes(ctx, query, nodes,
			func(n *Media) { n.Edges.Episodes = []*Episode{} },
			func(n *Media, e *Episode) { n.Edges.Episodes = append(n.Edges.Episodes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MediaQuery) loadEpisodes(ctx context.Context, query *EpisodeQuery, nodes []*Media, init func(*Media), assign func(*Media, *Episode)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(episode.FieldMediaID)
	}
	query.Where(predicate.Episode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(media.EpisodesColumn), fks...))
	}))
